...
function ordinary(owner: Player, value: number, id = `${owner}-${String(value)}`): Piece {
  return { id, value, owner, isDama: false };
}

function dama(owner: Player, value: number, id = `${owner}-dama-${String(value)}`): Piece {
  return { id, value, owner, isDama: true };
}

function only(moves: readonly Move[]): Move {
  if (moves.length !== 1) {
...
describe('game end by cornering, final scores include remaining chips (§7.3, §8, fixture 12)', () => {
  it('a player with a chip on the board but no legal move at all is cornered', () => {
    ...
    expect(finalScores(state)).toEqual({
      white: 10 + 3 + 6 * 2,
      black: 4 + 5,
    });
  });
...
describe('threefold repetition ends the game (§7.5, fixture 13)', () => {
  it('the third occurrence of the same position with the same player to move ends the game', () => {
    ...
    expect(isGameOver(positionP)).toBe(false);

    const wOut: Move = { from: { row: 7, col: 2 }, to: { row: 6, col: 1 }, captures: [] };
    const wBack: Move = { from: { row: 6, col: 1 }, to: { row: 7, col: 2 }, captures: [] };
    const bOut: Move = { from: { row: 0, col: 5 }, to: { row: 1, col: 4 }, captures: [] };
    const bBack: Move = { from: { row: 1, col: 4 }, to: { row: 0, col: 5 }, captures: [] };

    state = applyMove(state, wOut);
    state = applyMove(state, bOut);
    state = applyMove(state, wBack);
    state = applyMove(state, bBack); // occurrence 2, back to position P with white to move
    ...
    state = applyMove(state, bOut);
    state = applyMove(state, wBack);
    state = applyMove(state, bBack); // occurrence 3
    expect(state.board).toEqual(positionP.board);
    expect(isGameOver(state)).toBe(true);
